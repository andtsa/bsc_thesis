...
use parsing::parse_entry;
use parsing::pretty_print;
use rayon::iter::IntoParallelRefIterator;
use rayon::iter::ParallelIterator;
use runner::run_solver_on;
use verify::verify_result;
...
fn main() -> Result<()> {
    ...
    let results = inputs
        .par_iter()
        .map(|(e, i)| run_solver_on(e.clone(), i.clone()).map(|out| (out, i.clone())))
